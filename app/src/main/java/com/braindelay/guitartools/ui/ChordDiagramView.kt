...
fun ChordDiagramView(
    ...
    Canvas(modifier = modifier.size(72.dp, 92.dp)) {
        ...
        if (!isOpenPosition) {
            ...
            drawText(textMeasurer, label, topLeft = Offset(2f, topPad - m.size.height / 2f), style = style)
        }

        // Nut for open position
        if (isOpenPosition) {
        ...
        for (f in 0..DISPLAY_FRETS) {
            ...
            drawLine(onSurfaceColor.copy(alpha = 0.4f), Offset(leftPad, y), Offset(rightEdge, y), strokeWidth = 1f)
        }

        // String lines
        for (s in 0..5) {
            val x = leftPad + s * stringSpacing
            drawLine(onSurfaceColor.copy(alpha = 0.5f), Offset(x, topPad), Offset(x, bottomEdge), strokeWidth = 1f)
        }
        ...
        for (s in 0..5) {
            ...
            when {
                fret == null -> {
                    ...
                    drawLine(Color(0xFFE53935), Offset(x - 5f, y - 5f), Offset(x + 5f, y + 5f), strokeWidth = 2f)
                    drawLine(Color(0xFFE53935), Offset(x + 5f, y - 5f), Offset(x - 5f, y + 5f), strokeWidth = 2f)
                }
                fret == 0 -> {
                    val y = topPad - fretSpacing * 0.6f
