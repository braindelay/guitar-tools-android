...
private val helpSections = listOf(
    ...
    HelpSection(
        "scales", "Scales",
        "root note mode major minor dorian phrygian lydian mixolydian locrian harmonic melodic " +
                "pentatonic major-pentatonic minor-pentatonic blues b5 blue-note " +
                "fretboard roman numerals degrees collapse expand selector"
    ),
    HelpSection(
    ...
    HelpSection(
        ...
        "ear training drill interval scale degree chord quality listen replay score streak " +
                "ascending descending harmonic m2 M2 m3 M3 P4 TT P5 m6 M6 m7 M7 P8 octave"
    )
)
...
private fun SectionBlock(id: String) {
    when (id) {
        ...
        "scales" -> Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            BulletList(
                listOf(
                    "**Major Pentatonic** (1–2–3–5–6), **Minor Pentatonic** (1–b3–4–5–b7), and **Blues** (1–b3–4–b5–5–b7) are available alongside the seven-note modes.",
                    "Roman numerals are shown only for the available degrees (5 or 6 instead of 7). Equivalent intervals share colours with the parent diatonic mode; the **b5** in the Blues scale is highlighted in the error colour.",
                    ...
                    "**Templates** in the Progression screen fall back to the parallel major or minor when the active scale is pentatonic or blues."
                )
            )
            DegreeLegend()
        }
        ...
        "ear_training" -> Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
            SectionHeader("Ear Training")
            HelpImg(R.drawable.help_ear_training, "Ear Training screen")
            BodyText(
                "Three listen-and-pick drills for sharpening your ear — interval recognition, " +
                        "scale degree recognition, and chord quality recognition. " +
                        "All audio is generated by the same Karplus-Strong synthesiser used elsewhere in the app."
            )
            SubHeader("Drills")
            BulletList(
                listOf(
                    "**Interval** — two notes play; identify the interval between them.",
                    "**Scale Degree** — the root note plays, then a degree from the major scale; identify which degree (I–VII) you heard.",
                    "**Chord Quality** — a single chord voicing plays; identify its quality (Major, Minor, Dim, Aug, Maj7, Min7, Dom7, HalfDim)."
                )
            )
            SubHeader("Using the Drills")
            NumberedList(
                listOf(
